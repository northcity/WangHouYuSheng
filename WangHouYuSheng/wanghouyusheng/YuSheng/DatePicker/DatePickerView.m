...
//        [_sureBtn addTarget:self action:@selector(sureBtnClick) forControlEvents:UIControlEventTouchUpInside];
        ...
        _sureBtn.titleLabel.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:14];
        ...
        _cannelBtn.titleLabel.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:14];
        
        
        UILabel * titleLab = [[UILabel alloc]init];
        [toolView addSubview:titleLab];
        titleLab.frame = CGRectMake((ScreenWidth-220)/2, 12, 220, 17);
        titleLab.text = NSLocalizedString(@"请选择时间", nil) ;
        titleLab.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:14];
        ...
        if (@available(iOS 13.4, *)) {
            _datePickerView.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"];//新发现这里不会根据系统的语言变了
