...
- (void)viewDidLoad {
    ...
    CGFloat scanWH = 220;
    CGFloat scanX = (ScreenWidth - scanWH) * 0.5;
    CGFloat scanY = (ScreenHeight - scanWH) * 0.5;
        ...
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            if (granted) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self loadScanView];
                    [self initOtherUI];
                    self.navTitleLabel.text = @"扫码同步爱情";
                          ...
                          [self.rightBtn setTitleColor:PNCColorWithHex(0x1e1e1e) forState:UIControlStateNormal];
                          ...
                          self.rightBtn.frame = CGRectMake(ScreenWidth - 20 - 45, 22 + shiPeiH + (PCTopBarHeight - 22)/2 - 12.5, 45, 25);
                });
            } else {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey,id> *)info {
    ...
    if ([features count] <= 0) {
        [QrCodeReaderViewController alertWithTitle:@"识别结果为nil"];
    } else {
        CIQRCodeFeature *feature = [features firstObject];
        
        if (self.qrcodeValueBlock) {
            self.qrcodeValueBlock(feature.messageString);
            [self.navigationController popViewControllerAnimated:YES];
        }
        
//        [QrCodeReaderViewController alertWithTitle:feature.messageString];
