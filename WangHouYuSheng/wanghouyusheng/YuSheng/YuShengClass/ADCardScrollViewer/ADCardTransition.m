...
- (void)pushAnimation:(id<UIViewControllerContextTransitioning>)transitionContext {
    //起始页面
    WSYSMainViewController *fromVC = (WSYSMainViewController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    //目标页面
    ...
    NSArray *cellArray = [fromVC.wsVC.cardScrollViewer.collectionView visibleCells];
    for (int i = 0; i < cellArray.count; i++) {
        if (fromVC.wsVC.currentIndex == [cellArray[i] tag] - 1000) {
...
//    titleView.frame = CGRectMake(kAUTOWIDTH(62.5),kAUTOHEIGHT(134) - PCTopBarHeight + kAUTOHEIGHT(10),ScreenWidth, ScreenHeight - kAUTOHEIGHT(300));
    
    [cell.titleView convertRect:cell.titleView.bounds toView:containerView];
...
//        imageView.transform =  CGAffineTransformMakeScale(1.2, 1.2);
        [toVC.topImageView convertRect:toVC.topImageView.bounds toView:containerView];
...
- (void)popAnimation:(id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    WSYSMainViewController *toVC = (WSYSMainViewController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    //取到cell
    CardAnimationCell *cell;
    NSArray *cellArray = [toVC.wsVC.cardScrollViewer.collectionView visibleCells];
    for (int i = 0; i < cellArray.count; i++) {
        if (toVC.wsVC.currentIndex == [cellArray[i] tag] - 1000) {
            cell = (CardAnimationCell *)cellArray[i];
        }
    }
    ...
    UIView *imageView = containerView.subviews[1];
    //titleView
