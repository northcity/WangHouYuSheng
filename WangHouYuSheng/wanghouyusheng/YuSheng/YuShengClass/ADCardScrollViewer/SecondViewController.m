...
@interface SecondViewController () <UITableViewDelegate, UITableViewDataSource,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UITextViewDelegate>
{
...
- (void)buildTableHeadView {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:10 initialSpringVelocity:1 options:UIViewAnimationOptionCurveLinear animations:^{
            self.topImageView.frame =CGRectMake(0, 0, ScreenWidth,kAUTOWIDTH(300));
    ...
    titleLabel.frame = CGRectMake(kAUTOWIDTH(15), kAUTOHEIGHT(10), kAUTOWIDTH(1), kAUTOHEIGHT(44));
...
// 内容
    ...
    self.contentTextView.text = model.contentString;
...
//    }];
    ...
    self.colorTextView = [[UITextView alloc]initWithFrame: CGRectMake(ScreenWidth - kAUTOWIDTH(160), ScreenHeight - [self Suit:300] - kAUTOHEIGHT(100), kAUTOWIDTH(140), kAUTOHEIGHT(60))];
    ...
    self.colorTextView.text = model.colorString;
...
//    [MainTuWenViewController maDaKaiShiZhenDong];
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"RELOAD" object:self userInfo:nil];
        if (self.backBlock) {
            self.backBlock(self.index);
        }
        [self.navigationController popViewControllerAnimated:NO];
    }else{
        
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"注意" message:@"您已经编辑了一条新的往后余生，确定要保存吗？" preferredStyle:UIAlertControllerStyleAlert];
