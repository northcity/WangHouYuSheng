...
- (void)buildUI {
    ...
    //标题
    self.title = [UILabel labelWithTitle:@"" AndColor:@"515151" AndFont:16 AndAlignment:NSTextAlignmentLeft];
    self.title.numberOfLines = 1;
     self.title.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:18];
...
//        make.centerX.equalTo(self.mas_centerX);
    ...
    //价格
    self.price = [UILabel labelWithTitle:@"" AndColor:@"8a8a8a" AndFont:14 AndAlignment:NSTextAlignmentCenter];
    [self.titleView addSubview:self.price];
    self.price.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:14];
    self.price.textAlignment = NSTextAlignmentCenter;
    
    [self.price mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.title.mas_bottom).offset([self Suit:10]);
        make.left.equalTo(self.titleView.mas_left).offset([self Suit:15]);
        make.width.mas_equalTo(self.frame.size.width);
    }];
    
    self.favoriteLabel = [UILabel labelWithTitle:@"2018年1月20日" AndColor:@"8a8a8a" AndFont:14 AndAlignment:NSTextAlignmentCenter];
    
    self.favoriteLabel.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:10];
    [self.titleView addSubview:self.favoriteLabel];
    [self.favoriteLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.price.mas_bottom).offset([self Suit:20]);
        make.left.equalTo(self.titleView.mas_left).offset([self Suit:15]);
    ...
    self.indexLabel = [UILabel labelWithTitle:@"2018年1月20日" AndColor:@"ffffff" AndFont:14 AndAlignment:NSTextAlignmentCenter];
    self.indexLabel.alpha = 0.7;
    self.indexLabel.frame = CGRectMake(self.frame.size.width/2 -kAUTOWIDTH(20), self.frame.size.height + kAUTOHEIGHT(20), kAUTOWIDTH(40), kAUTOHEIGHT(22));
    self.indexLabel.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:9];
    [self.contentView addSubview:self.indexLabel];
    
     self.indexLabel.layer.shadowOffset =CGSizeMake(0, 0);
     self.indexLabel.layer.shadowColor = [UIColor grayColor].CGColor;
     self.indexLabel.layer.shadowRadius = 4;
     self.indexLabel.layer.shadowOpacity = 0.6;
    
    
    self.titleLabel = [UILabel labelWithTitle:@"2018年1月20日" AndColor:@"232323" AndFont:14 AndAlignment:NSTextAlignmentCenter];
    self.titleLabel.alpha = 0.4;
    self.titleLabel.frame = CGRectMake(0, CGRectGetMaxY(self.indexLabel.frame) + kAUTOHEIGHT(0), self.frame.size.width, kAUTOHEIGHT(22));
    self.titleLabel.font = [UIFont fontWithName:@"TpldKhangXiDictTrial" size:10];
...
//    self.indexLabel.backgroundColor = [UIColor redColor];
    ...
    self.titleLabel.layer.shadowColor = [UIColor grayColor].CGColor;
...
-(void)loadDateWithModel:(LZDataModel *)model{
    UIImage *image = [ChuLiImageManager decodeEchoImageBaseWith:model.pcmData];
    self.coverImage.image = image;
    self.title.text = model.titleString;
    self.price.text = model.contentString;
    self.favoriteLabel.text = model.colorString;
}
