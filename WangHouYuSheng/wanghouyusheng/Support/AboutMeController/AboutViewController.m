...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 8) {
        if (!self.wangHouYuShengBtn) {
            ...
            self.wangHouYuShengBtn = [self CellContentViewwWithBtnNormalString:@"去评价" WithBtnSelectedString:@"获取" WithAppUrlSecheme:@"comchenxiwanghouyusheng://" WithImageName:@"1024" WithTitleString: [NSString stringWithFormat:@"Version %@", [infoDictionary objectForKey:@"CFBundleShortVersionString"]] WithDetailString:@"余生有你" WithCell:cell];
            [self.wangHouYuShengBtn addTarget:self action:@selector(awakeSelfApp) forControlEvents:UIControlEventTouchUpInside];
        }
    }
    return  cell;
    }
