...
+ (FMDatabase*)LZDefaultDataBase {
    ...
    if (LZ_dbPath == nil) {
        [self LZCreateSqliteWithName:LZSqliteName];
    }
...
//        NSString *email = model.email;
        ...
        NSString *insert = [NSString stringWithFormat:@"INSERT INTO '%@' (identifier,groupName,nickName,userName,password,urlString,dsc,groupID,email) VALUES ('%@','%@','%@','%@','%@','%@','%@','%@','%@')",tableName,model.identifier,model.groupName,model.nickName,userName,psw,model.urlString,dsc,model.groupID,model.email];
...
//            model.email = [LZStringEncode decode:email];
            ...
            model.contentString = [fs stringForColumn:@"contentString"];
            model.colorString = [fs stringForColumn:@"colorString"];
            model.pcmData = [fs stringForColumn:@"pcmData"];
            model.email = [fs stringForColumn:@"email"];
